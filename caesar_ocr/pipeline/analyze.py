...
from .schemas import LayoutLMClassification, OcrDocument, OcrPage, PipelineResult
...
def analyze_document_bytes(
    ...
    *,
    ...
    if pages:
        first = pages[0]
        page_items.append(OcrPage(page=first.page, width=first.width, height=first.height, text=ocr_result.ocr_text))
    else:
